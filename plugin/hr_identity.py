...
``$HERMES_HOME/remote/`` and is never logged, printed or put in a QR code.
"""
...
#: Files under ``$HERMES_HOME/remote/``. PEM rather than DER so ``ssl_certfile``/``ssl_keyfile``
...
class Identity:
    ...
    def fingerprint_hex(self) -> str:
        """Lowercase hex SHA-256 of the DER, colon-free. What ``hermes remote status`` prints."""
...
def ensure_identity(*, regenerate: bool = False) -> Identity:
    ...
    ``hermes remote rotate`` would use — and which unpairs every device by design.
    """
...
def existing_identity() -> Optional[Identity]:
    ...
    ``hermes remote status`` uses this: reporting "no certificate yet" is the honest answer, and
